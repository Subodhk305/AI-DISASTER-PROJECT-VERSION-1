...
import { Package, AlertTriangle, CheckCircle, Truck, Warehouse, MapPin, Plus, Edit, Trash2 } from 'lucide-react';
...
export default function EquipmentInventoryPage() {
  const [equipment] = useState([
    ...
    { id: 7, name: 'Stretchers', quantity: 120, allocated: 35, threshold: 40, location: 'Medical Unit', status: 'available', category: 'Medical', lastUpdated: '2026-03-24' },
    { id: 8, name: 'Communication Radios', quantity: 50, allocated: 12, threshold: 15, location: 'Command Center', status: 'available', category: 'Equipment', lastUpdated: '2026-03-24' },
  ]);
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold text-white">Equipment Inventory</h1><p className="text-gray-400 text-sm">Manage emergency equipment and supplies inventory</p></div>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
        {categories.map(cat => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-1.5 rounded-lg text-sm transition-all ${selectedCategory === cat ? 'bg-[#00D4FF] text-gray-900' : 'bg-white/5 text-gray-400 hover:text-white'}`}>{cat}</button>))}
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full"><thead><tr className="border-b border-white/10"><th className="text-left py-3 text-gray-400">Item</th><th className="text-left py-3 text-gray-400">Category</th><th className="text-left py-3 text-gray-400">Quantity</th><th className="text-left py-3 text-gray-400">Allocated</th><th className="text-left py-3 text-gray-400">Available</th><th className="text-left py-3 text-gray-400">Location</th><th className="text-left py-3 text-gray-400">Status</th><th className="text-left py-3 text-gray-400">Actions</th></tr></thead>
          <tbody>{filteredEquipment.map(item => (<tr key={item.id} className="border-b border-white/10 hover:bg-white/5"><td className="py-3 text-white">{item.name}</td><td className="py-3 text-gray-400">{item.category}</td><td className="py-3 text-white">{item.quantity}</td><td className="py-3 text-white">{item.allocated}</td><td className="py-3 text-green-400">{item.quantity - item.allocated}</td><td className="py-3 text-gray-400 flex items-center gap-1"><MapPin size={12} />{item.location}</td><td className="py-3"><span className={`text-xs px-2 py-1 rounded-full ${item.status === 'available' ? 'text-[#00E57A] bg-[#00E57A]/10' : 'text-[#FF3B5C] bg-[#FF3B5C]/10'}`}>{item.status === 'available' ? 'Available' : 'Low Stock'}</span></td><td className="py-3"><div className="flex gap-2"><button className="p-1 text-gray-400 hover:text-[#00D4FF]"><Edit size={14} /></button><button className="p-1 text-gray-400 hover:text-red-400"><Trash2 size={14} /></button></div></td></tr>))}</tbody>
        </table>
      </div>
    </div>
  );
